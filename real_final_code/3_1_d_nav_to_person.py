...
class YoloPersonNavGoal(Node):
    def __init__(self):
        ...
        self.close_enough_distance = 2  # meters 2.0 -> 0.5
        self.block_goal_updates = False
        self.close_distance_hit_count = 0  # To avoid reacting to a single bad reading

        # Load YOLOv8 model
        # self.model = YOLO("/home/rokey-kim/Documents/ros2_ws/src/day3/day3/yolov8n.pt")
        self.model = YOLO("/home/rokey/ros2_ws/src/turtlebot4_test/turtlebot4_test/best.pt")
    ...
    def process_frame(self):
        ...
        for det in results.boxes:
            ...
            # cv2.putText(frame, f"{label} {conf:.2f}, z={z:.2f}m", (x1, y1 - 5),
            #     cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            ...
            if label.lower() == "employee":
                ...
                u = int((x1 + x2) // 2)
                v = int((y1 + y2) // 2)
                z = float(self.depth_image[v, u])/1000.0 -2
                self.get_logger().warn(f"Object distancez={z:.2f}m")
