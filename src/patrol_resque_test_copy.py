...
class PatrolNode(Node):
    def __init__(self):
        ...
        # 임시 camera_info (1280x720, focal 1000, 중심 640, 360)
        self.K = np.array([[1000, 0, 640], [0, 1000, 360], [0, 0, 1]], dtype=np.float32)

        self.webcam_thread_stop = threading.Event()
        ...
        self.get_logger().info('Patrol node (webcam test) initialized!')
    ...
    def webcam_loop(self):
        ...
        while not self.webcam_thread_stop.is_set():
            ret, frame = cap.read()
            print("웹캠 프레임 ret:", ret)  # <--- 추가
            if not ret:
                self.rgb_image = np.full((720, 1280, 3), 200, dtype=np.uint8)
                self.depth_image = np.full((720, 1280), 1000, dtype=np.uint16)
                self.update_display()
                cv2.waitKey(100)
                continue
            self.rgb_image = frame.copy()
            self.depth_image = np.full((frame.shape[0], frame.shape[1]), 1000, dtype=np.uint16)
