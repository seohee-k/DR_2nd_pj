...
class ResqueNode(Node):
    def __init__(self):
        """
        직원 대피(rescue) 노드 초기화
        - fire_flag 신호를 받으면 직원 대피 모드로 전환
        - employee_stack 토픽으로 받은 직원 좌표를 순차적으로 방문
        """
        super().__init__('resque_node', namespace='robot7')
        self.employee_stack = []      # 대피시킬 직원 좌표 리스트
        self.executing = False        # 현재 goal(이동) 실행 중 여부
        self.fire_flag = False        # 화재 신호 수신 여부

        # 화재 신호 구독 (fire_flag가 True가 되면 대피 시작)
        self.create_subscription(Bool, 'fire_flag', self.fire_flag_cb, 10)
        # 직원 좌표 구독 (patrol에서 publish한 좌표 수신)
        self.create_subscription(Point, 'employee_stack', self.employee_cb, 10)
        # 네비게이션 액션 클라이언트
        self.action_client = ActionClient(self, NavigateToPose, 'navigate_to_pose')
        # 주기적으로 직원 대피 함수 호출 (2초마다)
    ...
    def fire_flag_cb(self, msg):
        """
        fire_flag 콜백: 화재 신호 수신 시 대피 모드로 전환
        """
    ...
    def go_next_employee(self):
        """
        직원 대피 함수: fire_flag가 True이고, 이동 중이 아니며, 대피할 직원이 남아있으면
    ...
    def goal_response_callback(self, future):
        """
        네비게이션 goal 응답 콜백: goal이 수락되면 결과 콜백 등록
        """
    ...
    def result_callback(self, future):
        """
        네비게이션 goal 완료 콜백: 직원 대피 위치 도착 시 실행
        """
